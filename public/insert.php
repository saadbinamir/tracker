...
if (!empty($input['attributes']['ip'])) {
    if (filter_var($input['attributes']['ip'], FILTER_VALIDATE_IP)) {
        if (file_exists('../storage/app/blocked_ips/' . $input['attributes']['ip'])) {
            header("Connection: Close");
            die();
        }
    } else {
        unset($input['attributes']['ip']);
    }
}
