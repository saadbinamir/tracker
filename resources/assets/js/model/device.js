function Device(data) {
    ...
    _this.setCheckOffline = function(timestamp, timeout) {
        ...
        if (_online === 'offline' && options.online === 'blocked')
            return;
    ...
    _this.dataSensorsDOM = function( sensors ) {
        ...
        $.each( sensors, function(index, item){
            ...
            var _icon = '';
            if (_data.icon) {
                _icon = '<img class="icon sensor-icon" src="'+_data.icon+'" />';
            } else {
                var _iconClass = 'icon ' + item.type;

                if (item.scale_value !== null)
                    _iconClass += ' ' + item.type + '-' + item.scale_value;

                _icon = '<i class="' + _iconClass + '"></i>';
            }

            _row += '<td>'+ _icon + item.name + '</td>';
    ...
    _this.updateLayer = function () {
        ...
        if ( ! layer ) {
            ...
            if (_this.id()) {
                ...
                layer.options.device_id = _this.id();
            }
        } else {
            layer
                .setIcon( divIcon )
