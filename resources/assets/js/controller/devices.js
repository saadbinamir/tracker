function Devices() {
    ...
    _this.initCluster = function() {
        ...
        if ( cluster ) {
            cluster.options.disableClusteringAtZoom = app.settings.clusterDevice ? disableZoom : 1;

            return;
        }

        cluster = L.markerClusterGroup({
            disableClusteringAtZoom: app.settings.clusterDevice ? disableZoom : 1,
    ...
    _this.clusterPopup = function( cluster ) {
        var parametersHTML = '';

        parametersHTML += '<ul class="group-list">';
        $.each(cluster.getAllChildMarkers(), function (index, item) {
            var id = item.options.device_id;
            var device = app.devices.get(id);
            parametersHTML += '<li data-device-id="'+id+'" onclick="app.devices.get('+id+').onLayerClick();">';
            parametersHTML += '<div class="name">' + device.dataDOM("name") + '</div>';
            parametersHTML += '<div class="details">' + device.dataDOM("speed") + device.dataDOM("status") + '</div>';
            parametersHTML += '</li>';
        });
        parametersHTML += '</ul>';
        ...
        html += '   <div class="popup-body scrollbox">'+parametersHTML+'</div>';
        html += '</div>';
