...
namespace Tobuli\Helpers\Payments\Gateways\PayPal\Core;

use Tobuli\Helpers\Payments\Gateways\PayPal\Log\PayPalLogFactory;
...
class PayPalLoggingManager
{
    ...
    private function __construct($loggerName)
    {
        ...
        $factory = array_key_exists('log.AdapterFactory', $config) && in_array('Tobuli\Helpers\Payments\Gateways\PayPal\Log\PayPalLogFactory', class_implements($config['log.AdapterFactory'])) ? $config['log.AdapterFactory'] : '\Tobuli\Helpers\Payments\Gateways\PayPal\Log\PayPalDefaultLogFactory';
