...
class DeviceService extends AbstractEntity implements DisplayInterface
{
    use ChangeLogs;
    use DisplayTrait;
    use HasFactory;
    ...
    public function scopeExpireByOdometer($query, $value)
    {
        return $query
            ...
            ->whereRaw("((
                sensors.shown_value_by = 'virtual_odometer' AND ((sensors.odometer_value_unit = 'km' && sensors.value >= $value) OR (sensors.odometer_value_unit = 'mi' && (sensors.value * 0.621371192) >= $value))
                ) OR (
                sensors.shown_value_by = 'connected_odometer' AND sensors.value_formula >= $value
            ))")
            ->groupBy('device_services.id');
    }
